...
public class Smile_care_application {
	...
	public void insert() {
		...
		for(int i=0;i<size;i++) {
			...
			try {
				PreparedStatement ps=con.prepareStatement("insert into dental(patient_id,patient_name,type,cost) values(?,?,?,?)");
				ps.setInt(1, p_id);
	...
	public void disp() {
		try {
			PreparedStatement ps=con.prepareStatement("SELECT * from dental");
